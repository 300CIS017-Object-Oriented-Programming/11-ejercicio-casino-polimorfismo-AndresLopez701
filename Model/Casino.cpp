...
void Casino::agregarJugador() {
    ...
    // Se agrega jugador solo si no existe con anticipación
    do {
        cout << "Ingrese el id del jugador \n";
        cin >> idJugador;
        if (verExisteJugador(idJugador)) {
            cout << "El jugador con la identificación recibida ya existe\n";
            errores = true;
        } else {
            errores = false;
        }
...
/* Este metodo de ejemplo delega los mensajes de error en la interacción con la consola únicamente a la vista */
...
float Casino::jugar(int idJuego, long idJugador, float gonzosApostar) {
    ...
    if (!verExisteJugador(idJugador)) {
        throw std::domain_error("El jugador con la identificación recibida NO existe, no es posible jugar\n");
    }
...
void Casino::verInfoJugador(long idJugador) {
    ...
    if (!verExisteJugador(idJugador)) {
        throw std::domain_error("El jugador con la identificación recibida NO existe\n");
    }
    Jugador *pJugador = jugadoresMap[idJugador];
...
void Casino::retirarJugador(long idJugador) {
    if (verExisteJugador(idJugador)) {
        ...
        throw std::invalid_argument("El jugador con la identificación recibida NO existe\n");
    }
}
...
bool Casino::verExisteJugador(long id) {
    ...
    // Este es un tipo de condicional de una línea para guardar la variable del resultado
    bool result = (jugadoresMap.find(id) != jugadoresMap.end());
    return result;
}

/* OJO este metodo podría ser estático
...
// Nueva implementación de getJuegosDisponibles
vector<Juego*> Casino::getJuegosDisponibles() {
    return juegosDisponibles; // Devuelve el vector de juegos disponibles
}

Casino::~Casino() {
    // Se libera la memoria de los jugadores y de los juegos
    // Simplificación de la iteración en el mapa
    for (auto &it : jugadoresMap) {
        Jugador *jugadorTemp = jugadoresMap[it.first];
        delete jugadorTemp;
    }

    // Simplificación del bucle de los juegos
    for (auto juegoTemp : juegosDisponibles) {
        delete juegoTemp;
    }
    cout << "Terminé de llamar destructor de casino \n";
}
